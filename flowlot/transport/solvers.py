"""Discrete optimal-transport solvers with a common target-by-reference convention."""
...
def solve_transport(
    ...
    if target.ndim != 2 or reference.ndim != 2 or target.shape[1] != reference.shape[1]:
        raise ValueError("target and reference must be 2D with equal marker dimensions")
    ...
    if name == "hungarian":
        if len(target) != len(reference):
            raise ValueError("Hungarian matching requires equal target/reference cell counts")
    ...
    elif name in {"emd", "emd2", "pot"}:
        try:
            import ot
        except ImportError as error:
            raise ImportError("Exact EMD requires the POT dependency") from error
        ...
        try:
            ...
            raise ImportError("Sinkhorn requires the POT dependency") from error
